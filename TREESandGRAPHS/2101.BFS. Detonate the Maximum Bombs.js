// cargo el array de Edges con un radio sobre una lista de adjacencias grap tipo Map
...
class Solution {
  maximumDetonation(bombs) {
      ...
      let answer = 0;
      for (let i = 0; i < n; i++) {
          answer = Math.max(answer, this.bfs(i, graph));
      }
      return answer;
  }
  bfs(i, graph) {
      ...
      while (queue.length > 0) {
          let cur = queue.shift();
          //en la lista de adjacencia las bombas dentro del radio de explosión
